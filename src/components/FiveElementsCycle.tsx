...
const FiveElementsCycle: React.FC = () => {
  ...
  const isRelationshipActive = (f:string, t:string) =>
    activeRelationships.some(r=> (r.from===f&&r.to===t)||(r.from===t&&r.to===f));

  const getRelationshipType = (f:string, t:string) => {
    if (relationships.generation.some(r=>r.from===f&&r.to===t)) return 'generation';
    if (relationships.control   .some(r=>r.from===f&&r.to===t)) return 'control';
    return null;
  };
        ...
        <div className="w-full md:w-2/5">
          {activeElement
            ? elements.filter(e=>e.id===activeElement).map(el=>(
                <div key={el.id} className={classNames('p-4 rounded-lg border h-full', el.borderColor)}>
                  ...
                  <div className="mt-6">
                    ...
                    <ul className="text-sm space-y-2">
                      {relationships.generation.filter(r=>r.from===el.id||r.to===el.id)
                        .map((r,i)=>{
                          const other = elements.find(o=>o.id=== (r.from===el.id?r.to:r.from))!;
                          const isFrom = r.from===el.id;
                          return (
                            <li key={i} className="flex items-center">
                              <span className={`w-3 h-3 rounded-full mr-2 ${other.color}`}/>
                              {isFrom
                      ...
                      {relationships.control.filter(r=>r.from===el.id||r.to===el.id)
                        .map((r,i)=>{
                          const other = elements.find(o=>o.id=== (r.from===el.id?r.to:r.from))!;
                          const isFrom = r.from===el.id;
                          return (
